...
DEFAULT_AUDIO_DEVICE_BLOCK_SIZE = 50000
DEFAULT_AUDIO_DEVICE_FLUSH_SIZE = 25600

class vars():
    default_audio_block_size = 80000
    ...
    def write(self, buf):
        ...
        if underflow:
            logging.warning('SoundDeviceStream write underflow (size: %d)',
                            len(buf))
            '''try:
                vars.default_audio_block_size += 20000
                logging.warning('Setting DEFAULT_AUDIO_DEVICE_BLOCK_SIZE to '+str(vars.default_audio_block_size))
            except: 
                pass
        try:
            vars.default_audio_block_size =80000
        except:
            pass'''
...
@click.option('--audio-block-size',
              ...
              #default = vars.default_audio_block_size,
...
if __name__ == '__main__':
    try:
        ...
        try:
            h = open(os.getcwd()+'\\start.start', 'w+')
            h.close()
        except:
            pass
